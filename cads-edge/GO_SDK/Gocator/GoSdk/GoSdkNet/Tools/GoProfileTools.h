...
namespace Lmi3d
{
    namespace GoSdk
    {
        namespace Tools
        {
            ...
            /// <summary>Represents a profile X-Line tool.</summary>
            public ref class GoProfileXLine : public GoProfileTool
            {
                KDeclareClass(GoProfileXLine, GoProfileXLine)

                /// <summary>Initializes a new instance of the GoProfileXLine class with the specified Zen object handle.</summary>
                /// <param name="handle">Zen object handle.</param>
                GoProfileXLine(IntPtr handle)
                    : GoProfileTool(handle)
                {}

                /// <summary>Initializes a new instance of the GoProfileXLine class.</summary>
                /// <param name="sensor">A GoSensor instance.</param>
                GoProfileXLine(GoSensor^ sensor)
                {
                    ::GoProfileXLine handle = kNULL;

                    KCheck(::GoProfileXLine_Construct(&handle, KToHandle(sensor), kNULL));

                    Handle = handle;
                }

                /// <inheritdoc cref="GoProfileXLine(GoSensor^ sensor)" />
                /// <param name="allocator">Memory allocator</param>
                GoProfileXLine(GoSensor^ sensor, KAlloc^ allocator)
                {
                    ::GoProfileXLine handle = kNULL;

                    KCheck(::GoProfileXLine_Construct(&handle, KToHandle(sensor), KToHandle(allocator)));
                ...
                property bool RegionEnabled
                {
                    bool get()           { return KToBool(::GoProfileXLine_RegionEnabled(Handle)); }
                    void set(bool enable)  { KCheck(::GoProfileXLine_EnableRegion(Handle, enable)); }
                }

                /// <summary>The profile X-Line tool region.</summary>
                property GoProfileRegion^ Region
                {
                    GoProfileRegion^ get()           { return KToObject<GoProfileRegion^>(::GoProfileXLine_Region(Handle)); }
                }

                /// <summary>The maximum tracking speed.</summary>
                property k64f MaxTrackingSpeed
                {
                    k64f get()           { return (k64f) ::GoProfileXLine_MaxTrackingSpeed(Handle); }
                    void set(k64f value)  { KCheck(::GoProfileXLine_SetMaxTrackingSpeed(Handle, value)); }
                }

                /// <summary>The GoProfileXLine Z measurement.</summary>
                property GoProfileXLineZ^ ZMeasurement
                {
                    GoProfileXLineZ^ get()           { return KToObject<GoProfileXLineZ^>(::GoProfileXLine_ZMeasurement(Handle)); }
                }

                /// <summary>The GoProfileXLine validity measurement.</summary>
                property GoProfileXLineZ^ ValidityMeasurement
                {
                    GoProfileXLineZ^ get()           { return KToObject<GoProfileXLineZ^>(::GoProfileXLine_ValidityMeasurement(Handle)); }
                }
            };
            ...
            internal:
                static GoProfileTool^ GetProfileToolInstance(::GoProfileTool tool)
                {
                    ...
                    if (kObject_Is(tool, kTypeOf(GoProfileXLine))) { return KToObject<GoProfileXLine^>(tool); }
