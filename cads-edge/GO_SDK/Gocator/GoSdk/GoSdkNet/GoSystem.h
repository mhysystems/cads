...
namespace Lmi3d
{
    namespace GoSdk
    {
        ...
        public ref class GoSystem : public KObject
        {
            ...
            /// <param name="sensor">point to a GoSensor object to return newly created sensor.</param>
            void AddSensor(GoAddressInfo addressInfo, GoPortInfo portInfo, GoSensor^ sensor)
            {
                ::GoSensor s = KToHandle(sensor);
                ::GoAddressInfo ai = (::GoAddressInfo)addressInfo;
                ::GoPortInfo pi = (::GoPortInfo)portInfo;
                KCheck(::GoSystem_AddSensor(Handle, &ai, &pi, &s));
            }
