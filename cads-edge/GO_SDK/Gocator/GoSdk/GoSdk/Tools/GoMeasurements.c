...
GoFx(kStatus) GoMeasurements_NameTypeMapInit(void)
{
    ...
    kTry
    {
        // TODO: Can make this an array1 of array1. First array1 is array of tool names.
        //       Each tool name entry contains an array1 of measurement name to
        //       measurement type mapping info.
        //       Need to also find a fast way to search for a measurement type.
        //
        // NOTE: first parameter is a concatenation of tool name and measurement name.
        // NOTE: if adding or deleting entries, update the maximum table size
        //       "GO_MEASUREMENTS_MAP_TABLE_SIZE".
...
{
    if (GoMeasurements_NameTypeMapEntryCount() != 0)
    {
...
GoFx(kStatus) GoMeasurements_ParseType(const kChar* toolName, const kChar* measurementName, kType* type)
{
    ...
    kTry
    {
        ...
        // The called function will not create a table if it already exists.
        kTest(GoMeasurements_CreateNameTypeMap());
...
kBeginClassEx(Go, GoProfileXLineZ)
...
kBeginClassEx(Go, GoProfileXLineValidity)
