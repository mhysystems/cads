...
GoFx(kStatus) GoTool_VRead(GoTool tool, kXml xml, kXmlItem item)
{
    ...
    kCheck(GoTools_FormatToolType(kObject_Type(tool), toolType, kCountOf(toolType)));
    ...
    {
        kCheck(GoTool_ReadFeatures(tool, xml, kXml_ItemName(xml, item), features));
    }
...
GoFx(kStatus) GoTool_VWrite(GoTool tool, kXml xml, kXmlItem item)
{
    ...
    for (i = 0; i < kArrayList_Count(obj->measurements); i++)
    {
        ...
        kCheck(GoMeasurements_FormatType(kObject_Type(measurement), measurementName, kCountOf(measurementName)));
    ...
    for (i = 0; i < kArrayList_Count(obj->featureOutputs); i++)
    {
        ...
        kCheck(GoFeatures_FormatType(kObject_Type(feature), featureName, kCountOf(featureName)));
...
GoFx(kStatus) GoTool_ReadFeatures(GoTool tool, kXml xml, const kChar* toolName, kXmlItem features)
{
    ...
    for (i = 0; i < numFeatures; i++)
    {
        ...
        if (kSuccess(GoFeatures_ParseType(toolName, kXml_ItemName(xml, featureItem), &featureType)))
        {
    ...
    while (kXml_ChildCount(xml, features) < kArrayList_Count(obj->featureOutputs))    //arraylist is accessed directly due to refresh behavior when the remove measurement function is used
    {
...
GoFx(kStatus) GoToolUtil_ParseStreamOptions(GoTool tool, kXml xml, kXmlItem item, kArrayList list)
{
    ...
    if (kXml_ChildExists(xml, item, "StreamOptions"))
    {
        ...
        for (i = 0; i < kXml_ChildCount(xml, streamItem); i++)
        {
            ...
            kCheck(kXml_Attr32s(xml, tempItem, "step", &option.step));
            kCheck(kXml_Attr32s(xml, tempItem, "ids", &option.id));
