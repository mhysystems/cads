...
GoFx(kStatus) GoTools_Write(GoTools tools, kXml xml, kXmlItem item)
{
    ...
    for (i = 0; i < kArrayList_Count(obj->tools); i++)
    {
        ...
        kCheck(GoTools_FormatToolType(kObject_Type(currentTool), toolName, kCountOf(toolName)));
...
GoFx(kStatus) GoTools_ReadTools(GoTools tools, kXml xml, kXmlItem ToolListItem)
{
    ...
    for (i = 0; i < toolCount; i++)
    {
        ...
        if (kSuccess(GoTools_ParseToolType(toolName, &type)))
        {
...
GoFx(kStatus) GoTools_AddTool(GoTools tools, GoToolType type, GoTool* tool)
{
    ...
    case GO_TOOL_PROFILE_X_LINE: mapType = kTypeOf(GoProfileXLine); break;
...
GoFx(kStatus) GoTools_NamekTypeMapInit(void)
{
    ...
    kTry
    {
        ...
        // TODO: This profile xline support still needed?
        kTest(GoTools_NamekTypeMapAdd(GO_PROFILE_TOOL_NAME_XLINE, kTypeOf(GoProfileXLine)));
...
GoFx(kStatus) GoTools_CreateNamekTypeMap(void)
{
    ...
    if (GoTools_NamekTypeMapEntryCount() != 0)
    {
        // Map table already created.
        return kOK;
    }
...
{
    // This call won't create a table if one already exists.
...
GoFx(kStatus) GoTools_FormatToolType(kType toolType, kChar* toolName, kSize capacity)
{
    ...
    kCheck(GoTools_FormatToolTypeHelper(toolType, &output));
...
GoFx(kStatus) GoTools_CreateNameToolIdMap(void)
{
    ...
    if (GoTools_NameToolIdEntryCount() != 0)
    {
        // Map table already created.
        return kOK;
    }
    ...
    kCheck(GoTools_NameToolIdMapAdd(GO_PROFILE_TOOL_NAME_XLINE, GO_TOOL_PROFILE_X_LINE ));
...
GoFx(const kChar*) GoTools_BuiltInToolDefineToString(GoToolType type)
{
    ...
    kStatus status = GoTools_CreateNameToolIdMap();
