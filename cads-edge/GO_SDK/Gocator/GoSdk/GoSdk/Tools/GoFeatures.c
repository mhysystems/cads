...
GoFx(kStatus) GoFeatures_NameTypeMapInit(void)
{
    ...
    kTry
    {
        // NOTE: first parameter is a concatenation of tool name and feature name.
        // NOTE: if adding or deleting entries, update the maximum table size
        //       "GO_FEATURES_MAP_TABLE_SIZE".
...
{
    if (GoFeatures_NameTypeMapEntryCount() != 0)
    {
...
GoFx(kStatus) GoFeatures_ParseType(const kChar* toolName, const kChar* featureName, kType* type)
{
    ...
    kTry
    {
        ...
        kTest(GoFeatures_CreateNameTypeMap());
