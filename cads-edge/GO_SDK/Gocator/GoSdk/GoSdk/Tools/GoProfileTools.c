...
kBeginClassEx(Go, GoProfileXLine)
kAddVMethod(GoProfileXLine, kObject, VRelease)
kAddVMethod(GoProfileXLine, GoTool, VInit)
kAddVMethod(GoProfileXLine, GoTool, VRead)
kAddVMethod(GoProfileXLine, GoTool, VWrite)
kEndClassEx()

GoFx(kStatus) GoProfileXLine_Construct(GoProfileXLine* tool, kObject sensor, kAlloc allocator)
{
    return GoTool_Construct(tool, kTypeOf(GoProfileXLine), sensor, allocator);
}

GoFx(kStatus) GoProfileXLine_VInit(GoProfileXLine tool, kType type, kObject sensor, kAlloc alloc)
{
    kObjR(GoProfileXLine, tool);

    kCheck(GoProfileTool_Init(tool, type, GO_TOOL_PROFILE_X_LINE, sensor, alloc));
    obj->maxTrackingSpeed = 0.0;
    obj->regionEnabled = kFALSE;
    obj->region = kNULL;
    ...
    kCheck(GoTool_AddMeasurement(tool, kTypeOf(GoProfileXLineZ), kTRUE, kNULL));
    kCheck(GoTool_AddMeasurement(tool, kTypeOf(GoProfileXLineValidity), kTRUE, kNULL));

    return kOK;
}

GoFx(kStatus) GoProfileXLine_VRelease(GoProfileXLine tool)
{
    kObj(GoProfileXLine, tool);
...
GoFx(kStatus) GoProfileXLine_VRead(GoProfileXLine tool, kXml xml, kXmlItem item)
{
    kObj(GoProfileXLine, tool);
    kXmlItem regionItem = kNULL;
    ...
    kCheck(kXml_Child64f(xml, item, "MaxTrackingSpeed", &obj->maxTrackingSpeed));
...
GoFx(kStatus) GoProfileXLine_VWrite(GoProfileXLine tool, kXml xml, kXmlItem item)
{
    kObj(GoProfileXLine, tool);
    kXmlItem regionItem = kNULL;
    ...
    kCheck(kXml_SetChild64f(xml, item, "MaxTrackingSpeed", obj->maxTrackingSpeed));
...
GoFx(kBool) GoProfileXLine_RegionEnabled(GoProfileXLine tool)
{
    kObj(GoProfileXLine, tool);
...
GoFx(kStatus) GoProfileXLine_EnableRegion(GoProfileXLine tool, kBool enable)
{
    kObj(GoProfileXLine, tool);
...
GoFx(GoProfileRegion) GoProfileXLine_Region(GoProfileXLine tool)
{
    kObj(GoProfileXLine, tool);
...
GoFx(kStatus) GoProfileXLine_SetMaxTrackingSpeed(GoProfileXLine tool, k64f value)
{
    kObj(GoProfileXLine, tool);

    obj->maxTrackingSpeed = value;

    return kOK;
}

GoFx(k64f) GoProfileXLine_MaxTrackingSpeed(GoProfileXLine tool)
{
    kObj(GoProfileXLine, tool);

    return obj->maxTrackingSpeed;
}

GoFx(GoProfileXLineZ) GoProfileXLine_ZMeasurement(GoProfileXLine tool)
{
    return GoTool_FindMeasurementByType(tool, GO_MEASUREMENT_PROFILE_X_LINE_Z);
}

GoFx(GoProfileXLineValidity) GoProfileXLine_ValidityMeasurement(GoProfileXLine tool)
{
    return GoTool_FindMeasurementByType(tool, GO_MEASUREMENT_PROFILE_X_LINE_VALIDITY);
}
