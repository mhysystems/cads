...
GoFx(kStatus) GoDiscoveryExtInfo_Read(GoDiscoveryExtInfo msg, kSerializer serializer, kAlloc alloc)
{
    ...
    kTry
    {
        ...
        kTest(kSerializer_BeginRead(serializer, kTypeOf(k32u), kTRUE));

        kTest(kSerializer_Read32u(serializer, &temp));

        kTest(kSerializer_Read32u(serializer, &responseId));
        ...
        kTest(kSerializer_Read32s(serializer, &status));
        kTest(status);
        kTest(kSerializer_Read32u(serializer, &temp));

        kTest(kSerializer_Read64u(serializer, &signature));
