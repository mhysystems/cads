...
GoFx(kStatus) GoSetupNode_Init(GoSetupNode node, kType type, kObject setup, GoRole role, kObject sensor, kAlloc alloc)
{
    ...
    obj->colIndx = 0;
    obj->direction = 0;
    obj->multiplexingBank = 0;
    obj->multiplexingBankUsed = kFALSE;
...
GoFx(kStatus) GoSetupNode_ReadConfig(GoSetupNode node, kXml xml, kXmlItem item)
{
    ...
    if (!kIsNull(tempItem))
    {
        ...
        {
            kCheck(kXml_AttrBool(xml, tempItem, "used", &obj->multiplexingBankUsed));
        }
        if (kXml_AttrExists(xml, tempItem, "value"))
        {
            kCheck(kXml_Attr32s(xml, tempItem, "value", &obj->multiplexingBank));
        }
    }
    //buddy layout
    ...
    {
        layoutItem = kXml_Child(xml, item, "Layout");
    ...
    if (obj->gridUsed)
    {
        ...
        kCheck(kXml_Attr32s(xml, tempItem, "value", &obj->rowIndx));
        tempItem = kXml_Child(xml, layoutItem, "Column");
        kCheck(kXml_Attr32s(xml, tempItem, "value", &obj->colIndx));
        tempItem = kXml_Child(xml, layoutItem, "Direction");
        kCheck(kXml_Attr32u(xml, tempItem, "value", &obj->direction));
    }
...
GoFx(kStatus) GoSetupNode_WriteConfig(GoSetupNode node, kXml xml, kXmlItem item)
{
    ...
    //what if older version I assume it would ignore
    //kCheck(kXml_SetChild32s(xml, item, "index", obj->index));
    ...
    kCheck(kXml_SetItem32s(xml, rowItem, obj->rowIndx));
    kCheck(kXml_SetAttr32s(xml, rowItem, "value", obj->rowIndx));
    kCheck(kXml_AddItem(xml, griditem, "Column", &columnItem));
    kCheck(kXml_SetItem32s(xml, columnItem, obj->colIndx));
    kCheck(kXml_SetAttr32s(xml, columnItem, "value", obj->colIndx));
    kCheck(kXml_AddItem(xml, griditem, "Direction", &directionItem));
    kCheck(kXml_SetAttr32s(xml, directionItem, "value", obj->direction));
    kCheck(kXml_SetItem32s(xml, directionItem, obj->direction));

    kCheck(kXml_AddItem(xml, layoutItem, "MultiplexingBank", &multiplexbank));
    kCheck(kXml_SetAttrBool(xml, multiplexbank, "used", obj->multiplexingBankUsed));
    kCheck(kXml_SetAttr32s(xml, multiplexbank, "value", obj->multiplexingBank));
    kCheck(kXml_SetItem32s(xml, multiplexbank, obj->multiplexingBank));
...
GoFx(kStatus) GoSetupNode_SetMultiplexingBankUsed(GoSetup setup, kBool value)
{
    kObj(GoSetupNode, setup);

    return kOK;
}
GoFx(kStatus) GoSetupNode_SetMultiplexingBank(GoSetup setup, k64f value)
{
    kObj(GoSetupNode, setup);

    return kOK;
}
GoFx(kStatus) GoSetupNode_SetLayoutGridUsed(GoSetup setup, kBool value)
{
    kObj(GoSetupNode, setup);

    return kOK;
}
GoFx(kStatus) GoSetupNode_SetLayoutGridDirection(GoSetup setup, k64f value)
{
    kObj(GoSetupNode, setup);

    return kOK;
}
GoFx(kStatus) GoSetupNode_SetLayoutGridColumn(GoSetup setup, k64f value)
{
    kObj(GoSetupNode, setup);

    return kOK;
}
GoFx(kStatus) GoSetupNode_SetLayoutGridRow(GoSetup setup, k64f value)
{
    kObj(GoSetupNode, setup);
...
GoFx(kStatus) GoSetup_ReadConfig(GoSetup setup, kXml xml, kXmlItem item)
{
    ...
    while (!kIsNull(deviceItem))
    {
        ...
        kCheck(kXml_Attr32u(xml, deviceItem, "index", &tempNodeIndex));
...
/*
GoFx(kStatus) GoSetup_SetLayoutMultiplexingBankUsed(GoSetup setup, GoRole role, kBool value)
{
    ...
    obj->multiplexingBankUsed = value;
    kCheck(GoSensor_SetConfigModified(setupObj->sensor));

    return kOK;
}
*/
GoFx(kStatus) GoSetup_SetLayoutMultiplexingBank(GoSetup setup, GoRole role, k32s value)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    obj->multiplexingBank = value;
    kCheck(GoSensor_SetConfigModified(setupObj->sensor));

    return kOK;
}

GoFx(kStatus) GoSetup_SetLayoutGridUsed(GoSetup setup, GoRole role, kBool value)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
/*
GoFx(kStatus) GoSetup_SetLayoutGridDirectionSystemValue(GoSetup setup, GoRole role, k32s value)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
GoFx(kStatus) GoSetup_SetLayoutGridColumnSystemValue(GoSetup setup, GoRole role, k32s value)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
GoFx(kStatus) GoSetup_SetLayoutGridRowSystemValue(GoSetup setup, GoRole role, k32s value)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
GoFx(kBool) GoSetup_LayoutMultiplexingBankUsed(GoSetup setup, GoRole role)
{
    ...
    return obj->multiplexingBankUsed;
}

GoFx(k32s) GoSetup_LayoutMultiplexingBank(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
GoFx(k32s) GoSetup_LayoutGridDirectionSystemValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
GoFx(kStatus) GoSetup_LayoutGridColumnSystemValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
...
GoFx(kStatus) GoSetup_LayoutGridRowSystemValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
