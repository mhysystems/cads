...
GoFx(kStatus) GoDiscovery_ParseGetAddressReply(GoDiscovery discovery, GoDiscoveryInfo* info, kIpEndPoint* remoteEndPoint)
{
    ...
    kTry
    {
        ...
        kTest(kSerializer_BeginRead(obj->reader, kTypeOf(k64u), kTRUE));
    ...
    kFinally
    {
        // Automatically flush any unread data and free allocated serializer memory.
    ...
    kTry
    {
        while (kSuccess(kSocket_Wait(kUdpClient_Socket(obj->receiver), 0)))
        {
            ...
            if (remoteEndPoint.port == GO_DISCOVERY_PORT)
            {
                ...
                // a GetExtendedInfo reply. Rather than restructuring the code to read reply ID first,
                // we take a shortcut to attempt to read the reply as both types if needed.
                if (kSuccess(GoDiscovery_ParseGetAddressReply(discovery, &info, &remoteEndPoint)))
                {
                    kTest(GoDiscovery_ProcessGetAddressReply(discovery, &info));
                ...
                {
                    kTest(kStream_Seek(obj->readerBuffer, 0, kSEEK_ORIGIN_BEGIN));

                    if (kSuccess(GoDiscovery_ParseGetInfoReply(discovery, &extInfo, kObject_Alloc(discovery))))
                    {
                        kTest(GoDiscovery_ProcessGetExtendedInfoReply(discovery, extInfo, &remoteEndPoint));
...
GoFx(kStatus) GoDiscovery_OnEnumElapsedRecv(GoDiscovery discovery, kPeriodic timer)
{
    ...
    kTry
    {
        ...
        kTest(kPeriodic_Start(obj->eventTimer, kTimer_Remaining(obj->stopwatch), GoDiscovery_OnEnumElapsed, discovery));
