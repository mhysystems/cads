...
// KApiLib.cpp
// 
// Copyright (C) 2014-2019 by LMI Technologies Inc.
...
kStatus kCall Lmi3d::Zen::KApiLib_OnAlloc(kPointer provider, kSize size, void* mem)
{
    kStatus status = kOK;
    ...
    try
    {
        ...
        *(void**)mem = kNULL;
        ...
        if (size > 0)
        {
            ...
            KCheck(xkDefaultMemAlloc(provider, totalAllocationSize, &allocatedMem));
            ...
            *(kSize*)allocatedMem = size;

            *(void**)mem = kPointer_ByteOffset(allocatedMem, headerSize);
        }
...
kStatus kCall Lmi3d::Zen::KApiLib_OnFree(kPointer provider, void* mem)
{
    ...
    try
    {
        ...
        {
            kSize headerSize = KApiLib::MemHeaderSize;
            void* allocatedMem = kPointer_ByteOffset(mem, -1 * (kSSize)headerSize);   //step back to allocation header
            kSize size = *(kSize*)allocatedMem;                        //read out the allocated memory size
            kSize totalAllocationSize = headerSize + size;
