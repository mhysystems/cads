...
kFx(kStatus) xkDebugAlloc_Init(kDebugAlloc object, kType type, const kChar* name, kAlloc innerAlloc, kAlloc alloc)
{
    ...
    obj->base.canGetObject = kAlloc_CanGetObject(innerAlloc);
    obj->base.isForeign = kAlloc_IsForeign(innerAlloc);
...
kFx(kStatus) kDebugAlloc_Clear(kDebugAlloc object)
{
    ...
    kTry
    {
        ...
        while (!kIsNull(it))
        {
            ...
            kTest(kAlloc_Free(obj->innerAlloc, allocatedMem)); 
...
kFx(kStatus) xkDebugAlloc_VGet(kDebugAlloc object, kSize size, void* mem)
{
    kObj(kDebugAlloc, object); 
    ...
    {   
        ...
        kTest(kAlloc_Get(obj->innerAlloc, size + 2*padSize, &allocatedMem)); 
        ...
        kItemCopy(allocatedMem, padData, padSize); 
        kItemCopy(allocatedMem + padSize + size, padData, padSize); 
