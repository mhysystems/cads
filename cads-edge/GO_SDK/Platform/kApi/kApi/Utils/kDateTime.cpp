...
kBeginValueEx(k, kDateTime)
    ...
    kAddPrivateVersion(kDateTime, "kdat6", "7.0.0.0", "kDateTime-0", Write, Read)
...
{
    k64s utcWindowsTicks = 10*((dateTime - kDATE_TIME_EPOCH_WINDOWS) / 1000000 * 1000000);  //seconds only; handle microseconds below
    FILETIME utcFileTime; 
...
kFx(kStatus) kDateTime_LocalParts(kDateTime dateTime, k32s* year, k32s* month, k32s* day, k32s* dayOfWeek, k32s* hour, k32s* minute, k32s* second, k32s* microsecond, k32s* utcOffset)
{
    time_t rawTime = (dateTime - kDATE_TIME_EPOCH_UNIX) / 1000000;   //seconds
