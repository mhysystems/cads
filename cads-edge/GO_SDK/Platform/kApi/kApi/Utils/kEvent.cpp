...
kBeginClassEx(k, kEvent)
    ...
    kAddPrivateVMethod(kEvent, kObject, VInitClone)
kEndClassEx()
...
kFx(kStatus) xkEvent_VInitClone(kEvent evnt, kEvent source, kAlloc allocator)
{
    kObjR(kEvent, evnt); 
    kStatus status = kOK; 

    kCheck(xkEvent_Init(evnt, kObject_Type(source), allocator)); 

    kTry
    {
        kTest(kList_Assign(obj->listeners, kEvent_Listeners(source))); 
    }
    kCatch(&status); 
    {
        xkEvent_VRelease(evnt); 
