 ...
 *
 * @internal
 * Copyright (C) 2013-2019 by LMI Technologies Inc.  All rights reserved.
 */
...
kFx(kStatus) xkPoolAlloc_VGet(kPoolAlloc object, kSize size, void* mem)
{
    kObj(kPoolAlloc, object); 
    ...
    {   
        ...
        else if (size <= obj->maxManagedBufferSize)
        {
            ...
            if (rankInfo->isBlockBased)
            {
                ...
                if (rankInfo->freeBufferList.count > 0)
                {
                    ...
                    *(void**)mem = kPointer_ByteOffset(buffer, sizeof(xkPoolAllocBufferHeader));     
                    isAllocated = kTRUE; 
                }
            }
        ...
        if (!isAllocated)
        {
            kTest(xkPoolAlloc_AllocUnmanagedBuffer(object, size, mem));
        }
    }
    kFinally
    {
...
kFx(kStatus) xkPoolAlloc_VFree(kPoolAlloc object, void* mem)
{
    ...
    {   
        if (!kIsNull(mem))
        {
            xkPoolAllocBufferHeader* buffer = (xkPoolAllocBufferHeader*) kPointer_ByteOffset(mem, -1*(kSSize)sizeof(xkPoolAllocBufferHeader)); 
...
kFx(kStatus) xkPoolAlloc_AllocUnmanagedBuffer(kPoolAlloc object, kSize size, void* mem)
{
    kObj(kPoolAlloc, object); 
    kAlloc innerAlloc = kObject_Alloc(object); 
