...
kBeginClassEx(k, kBackTrace)
    kAddPrivateVMethod(kBackTrace, kObject, VInitClone)
...
kFx(kStatus) xkBackTrace_VInitClone(kBackTrace trace, kBackTrace source, kAlloc allocator)
{
    kObjR(kBackTrace, trace); 
    kObjN(kBackTrace, sourceObj, source);

    kCheck(kObject_Init(trace, kTypeOf(kBackTrace), allocator)); 
