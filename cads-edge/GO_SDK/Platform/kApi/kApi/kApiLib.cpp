...
kFx(kStatus) kApiLib_InvokeEarlyLogHandler(const kChar* message)
{
    kApiLogFx function = (kApiLogFx)xkApiLib_earlyLogCallback.function;

    if (!kIsNull(function))
    {
        function(xkApiLib_earlyLogCallback.receiver, message);
    }
...
kFx(kStatus) kApiLib_InvokeLogListeners(const kChar* message)
{
    kStaticObj(kApiLog); 
    ...
    while (!kIsNull(it))
    {
        ...
        function(listener->receiver, message); 
...
kFx(kStatus) xkApiLib_Log(const kChar* message)
{
    ...
    else
    {
        ...
        kLock_Enter(sobj->lock);
        {
            ...
            const kChar* formattedMessage =  kApiLib_InvokeLogFormatters(message);
         
            //notify listeners with final formatted content
            kApiLib_InvokeLogListeners(formattedMessage);
        }
        kLock_Exit(sobj->lock);
    }
