 ...
 * 
 * To support cloning, derived classes should override the kObject_VInitClone method. 
 * 
 ...
 * @see                 kObject_Dispose
 */
kInlineFx(kStatus) kObject_Clone(kObject* object, kObject source, kAlloc allocator)
{
    if (!kIsNull(source))
    {
        return xkObject_CloneImpl(object, source, allocator);
    }
...
{
    return xkObject_VTable(object)->VHasForeignData(object);
}
 ...
 * 
 * There is no requirement to call kObject_VInitClone from derived VInitClone methods.
 *
 * @protected           @memberof kObject
 * @param   object      Object instance (not yet initialized). 
...
/** 
 * Protected virtual method that reports whether the object, including aggregated child elements, contains any foreign memory references.   
 * 
 * @protected           @memberof kObject
 * @param   object      Object. 
 * @return              kTRUE if the object references memory allocated in a foreign address space. 
 */
kInlineFx(kBool) kObject_VHasForeignData(kObject object)
{
    return kFALSE; 
}
