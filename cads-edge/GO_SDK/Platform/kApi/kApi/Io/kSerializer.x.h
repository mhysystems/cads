...
typedef kStatus (kCall* kSerializerObjectDeserializeFx)(kObject object, kSerializer serializer, kAlloc allocator); 
...
//[Protected]
kInlineFx(kStatus) kSerializer_InvokeObjectDeserializer(kSerializer serializer, kType type, kTypeVersion version, kObject object, kAlloc alloc)
{
    kSerializerObjectDeserializeFx method = (kSerializerObjectDeserializeFx) kType_VersionDeserializeFx(type, version);

    return method(object, serializer, alloc); 
}
