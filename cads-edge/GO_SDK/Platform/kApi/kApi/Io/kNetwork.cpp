...
kFx(kStatus) xkNetworkAdapter_Construct(kNetworkAdapter* adapter, kNetworkInfo info, const kChar* name, kSize id, kBool isUp, kAlloc allocator)
{
    kAlloc alloc = kAlloc_Fallback(allocator);
    kStatus status; 
    ...
    if (!kSuccess(status = xkNetworkAdapter_Init(*adapter, kTypeOf(kNetworkAdapter), info, name, id, isUp, alloc)))
    {
...
kFx(kStatus) xkNetworkAdapter_Init(kNetworkAdapter adapter, kType type, kNetworkInfo info, const kChar* name, kSize id, kBool isUp, kAlloc alloc)
{
...
kFx(kStatus) xkNetworkInfo_AddAdapter(kNetworkInfo info, const kChar* name, kSize id, kBool isUp, kNetworkAdapter* adapter)
{    
    kObj(kNetworkInfo, info); 
    ...
    kTry
    {
        kTest(xkNetworkAdapter_Construct(&newAdapter, adapter, name, id, isUp, kObject_Alloc(info)));
...
kFx(kStatus) xkNetwork_DefaultQueryNetInfo(kNetworkInfo info)
{
    ...
    kTry
    {
        ...
        while (!kIsNull(it))
        {
            ...
            kTest(xkNetworkInfo_AddAdapter(info, name, it->IfIndex, isUp, &adapter));
        ...
        while (!kIsNull(ifaListIt))
        {
            if ((ifaListIt->ifa_addr != kNULL) && (ifaListIt->ifa_addr->sa_family == AF_INET))
            {
                ...
                if (!kSuccess(kNetworkInfo_FindAdapterByName(info, name, &adapter)))
                {
                    ...
                    kTest(xkNetworkInfo_AddAdapter(info, name, id, isUp, &adapter));
                }
