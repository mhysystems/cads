...
kFx(kStatus) kDat6Serializer_VReadObject(kDat6Serializer serializer, kObject* object, kAlloc allocator)
{
    ...
    kTry
    {
        ...
        if (command == xkDAT6_SERIALIZER_CMD_NULL)
        {
            *object = kNULL; 
        }
        else if ((command == xkDAT6_SERIALIZER_CMD_OBJECT) || (command == xkDAT6_SERIALIZER_CMD_COMPRESSED_OBJECT))
        {
            kType type = kNULL; 
            kTypeVersion version = kNULL; 
            kStatus status; 
            ...
            kTest(kAlloc_GetObject(alloc, type, object)); 

            if (!kSuccess(status = kSerializer_InvokeObjectDeserializer(serializer, type, version, *object, alloc)))    
            {
