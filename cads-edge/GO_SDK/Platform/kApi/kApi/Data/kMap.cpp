 ...
 *
 * @internal
 * Copyright (C) 2012-2019 by LMI Technologies Inc.
...
kBeginClassEx(k, kMap) 
    ...
    kAddPrivateVersion(kMap, "kdat6", "5.7.1.0", "kMap-0", WriteDat6V0, ReadDat6V0)

    //virtual methods
    kAddPrivateVMethod(kMap, kObject, VRelease)
    kAddPrivateVMethod(kMap, kObject, VDisposeItems)
    kAddPrivateVMethod(kMap, kObject, VInitClone)
    kAddPrivateVMethod(kMap, kObject, VSize)
    kAddPrivateVMethod(kMap, kObject, VHasForeignData)
...
kFx(kStatus) xkMap_VInitClone(kMap map, kMap source, kAlloc allocator)
{ 
    kObjR(kMap, map); 
    ...
    kCheck(xkMap_Init(map, kObject_Type(source), sourceObj->keyField.type, sourceObj->valueField.type, sourceObj->count, allocator)); 
    ...
    kTry
    {
        ...
        while (!kIsNull(item))
        {
            ...
            {
                kTest(kObject_Clone(&keyClone, *(kObject*)key, kObject_Alloc(map))); 
                key = &keyClone; 
            }
            if (xkMap_ValueIsRef(source))
            {
                kTest(kObject_Clone(&valueClone, *(kObject*)value, kObject_Alloc(map))); 
                value = &valueClone; 
            }
...
kFx(kBool) xkMap_VHasForeignData(kMap map)
{
    kObj(kMap, map);   
    kBool keyIsRef = kType_IsReference(obj->keyField.type); 
    kBool valIsRef = kType_IsReference(obj->valueField.type); 
    ...
    if (keyIsRef || valIsRef)
    {
        ...
        while (!kIsNull(item))
        {
            if (keyIsRef)   
            {
                ...
                if (!kIsNull(key) && kObject_HasForeignData(key))
                {
                    return kTRUE;
                }
            }
            ...
            if (valIsRef)   
            {
                ...
                if (!kIsNull(value) && kObject_HasForeignData(value))
                {
                    return kTRUE;
                }
            }
...
kFx(kStatus) xkMap_ReadDat6V0(kMap map, kSerializer serializer, kAlloc allocator)
{
    kObjR(kMap, map);
    ...
    kCheck(xkMap_Init(map, kTypeOf(kMap), keyType, valueType, count, allocator)); 
    ...
    {         
        ...
        {   
            ...
            kTest(kSerializer_ReadItems(serializer, obj->keyField.type, keyVersion, (void*)kMap_Key(map, itemObj), 1)); 
            kTest(kSerializer_ReadItems(serializer, obj->valueField.type, valueVersion, kMap_Value(map, itemObj), 1)); 
        }

        obj->count = count; 

        kTest(xkMap_Rehash(map)); 
    }
    kCatch(&status)
    {
        xkMap_VDisposeItems(map); 
