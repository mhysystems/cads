...
kBeginClassEx(k, kBox) 
    ...
    kAddPrivateVersion(kBox, "kdat5", "5.0.0.0", "29-1", WriteDat5V1, ReadDat5V1)
    kAddPrivateVersion(kBox, "kdat6", "5.7.1.0", "kBox-0", WriteDat6V0, ReadDat6V0)

    //virtual methods
    kAddPrivateVMethod(kBox, kObject, VRelease)
    kAddPrivateVMethod(kBox, kObject, VInitClone)
...
kFx(kStatus) xkBox_VInitClone(kBox box, kBox source, kAlloc allocator)
{
    kObjR(kBox, box);
    kObjN(kBox, sourceObj, source);
    kStatus status = kOK; 

    kCheck(kObject_Init(box, kObject_Type(source), allocator));     
...
kFx(kStatus) xkBox_ReadDat5V1(kBox box, kSerializer serializer, kAlloc allocator)
{
    kObjR(kBox, box);
    kSize length; 
    kTypeVersion itemVersion;
    ...
    kCheck(xkBox_Init(box, kTypeOf(kBox), itemType, allocator)); 

    if (!kSuccess(status = kSerializer_ReadItems(serializer, itemType, itemVersion, obj->item, 1)))
    {
        xkBox_VRelease(box); 
        return status;
    }
...
kFx(kStatus) xkBox_ReadDat6V0(kBox box, kSerializer serializer, kAlloc allocator)
{
    kObjR(kBox, box);
    kType itemType = kNULL;            
    kTypeVersion itemVersion; 
    ...
    kTry
    {
        kTest(kSerializer_ReadItems(serializer, itemType, itemVersion, obj->item, 1)); 
    }
    kCatch(&status)
    {
        xkBox_VRelease(box); 
