 ...
 *
 * @internal
 * Copyright (C) 2004-2019 by LMI Technologies Inc.
...
kBeginClassEx(k, kXml)
    ...
    kAddPrivateVersion(kXml, "kdat6", "6.0.0.0", "kXml-0", WriteDat6V0, ReadDat6V0)
    
    kAddPrivateVMethod(kXml, kObject, VRelease)
    kAddPrivateVMethod(kXml, kObject, VInitClone)
kEndClassEx() 
...
kFx(kStatus) xkXml_VInitClone(kXml xml, kXml source, kAlloc allocator)
{
    kStatus status; 

    kCheck(xkXml_Init(xml, allocator));

    if (!kSuccess(status = kXml_Assign(xml, source)))
    {
        xkXml_VRelease(xml); 
        return status;
    }
...
kFx(kStatus) xkXml_ReadDat6V0(kXml xml, kSerializer serializer, kAlloc allocator)
{
    kString string = kNULL; 
    kStatus status; 
...
const kChar* xkXml_Format32f(kXml xml, k32f val)
{
    ...
    if (!kSuccess(xkStrFormat32f(val, obj->formatBuffer, kCountOf(obj->formatBuffer), 10, kNULL)))
    {
        return kNULL;
    }
...
const kChar* xkXml_Format64f(kXml xml, k64f val)
{
    ...
    if (!kSuccess(xkStrFormat64f(val, obj->formatBuffer, kCountOf(obj->formatBuffer), 10, kNULL)))
    {
        return kNULL;
    }
