 ...
 *
 * @internal
 * Copyright (C) 2013-2019 by LMI Technologies Inc.
...
kBeginClassEx(k, kList) 
    ...
    kAddPrivateVersion(kList, "kdat6", "6.0.0.0", "kList-0", WriteDat6V0, ReadDat6V0)

    //virtual methods
    kAddPrivateVMethod(kList, kObject, VRelease)
    kAddPrivateVMethod(kList, kObject, VDisposeItems)
    kAddPrivateVMethod(kList, kObject, VInitClone)
    kAddPrivateVMethod(kList, kObject, VSize)
    kAddPrivateVMethod(kList, kObject, VHasForeignData)
...
kFx(kStatus) xkList_VInitClone(kList list, kList source, kAlloc allocator)
{ 
    kObjR(kList, list);
    ...
    kCheck(xkList_Init(list, kObject_Type(source), sourceObj->contentField.type, sourceObj->count, allocator)); 
    ...
    kTry
    {
        ...
        while (!kIsNull(item))
        {
            ...
            {
                kTest(kObject_Clone(&contentClone, *(kObject*)content, kObject_Alloc(list))); 
...
kFx(kBool) xkList_VHasForeignData(kList list)
{
    kObj(kList, list);
    ...
    if (xkList_ItemIsRef(list))
    {
        ...
        while (!kIsNull(item))
        {
            ...
            if (!kIsNull(content) && kObject_HasForeignData(content))
            {
                return kTRUE;
            }
...
kFx(kStatus) xkList_ReadDat6V0(kList list, kSerializer serializer, kAlloc allocator)
{
    kObjR(kList, list); 
    ...
    kCheck(xkList_Init(list, kTypeOf(kList), contentType, count, allocator)); 
    ...
    {         
        ...
        {   
            ...
            kTest(kSerializer_ReadItems(serializer, obj->contentField.type, contentVersion, (void*)xkList_ItemContent(list, itemObj), 1)); 
        }
