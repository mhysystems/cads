...
kBeginClassEx(k, kArrayList) 
    ...
    kAddPrivateVMethod(kArrayList, kObject, VInitClone)
    kAddPrivateVMethod(kArrayList, kObject, VSize)
    kAddPrivateVMethod(kArrayList, kObject, VHasForeignData)
...
kFx(kStatus) xkArrayList_VInitClone(kArrayList list, kArrayList source, kAlloc allocator)
{
    kObjR(kArrayList, list);
    kObjN(kArrayList, sourceObj, source);
    kStatus status = kOK; 

    kCheck(kObject_Init(list, kObject_Type(source), allocator));     
    ...
    kTry
    {
        ...
        kTest(kCloneItems(obj->itemType, obj->items, sourceObj->items, obj->count, kObject_Alloc(list))); 
    }
    kCatch(&status)
    {
...
kFx(kBool) xkArrayList_VHasForeignData(kArrayList list)
{
    kObj(kArrayList, list); 

    return kHasForeignData(obj->itemType, obj->items, obj->count);
}
