...
kBeginClassEx(k, kQueue) 
    ...
    kAddPrivateVersion(kQueue, "kdat5", "5.0.0.0", "42-1", WriteDat5V1, ReadDat5V1)
    kAddPrivateVersion(kQueue, "kdat6", "5.7.1.0", "kQueue-0", WriteDat6V0, ReadDat6V0)

    //virtual methods
    kAddPrivateVMethod(kQueue, kObject, VRelease)
    kAddPrivateVMethod(kQueue, kObject, VDisposeItems)
    kAddPrivateVMethod(kQueue, kObject, VInitClone)
    kAddPrivateVMethod(kQueue, kObject, VSize)
    kAddPrivateVMethod(kQueue, kObject, VHasForeignData)
...
kFx(kStatus) xkQueue_VInitClone(kQueue queue, kQueue source, kAlloc allocator)
{
    kObjR(kQueue, queue);  
    ...
    kCheck(kObject_Init(queue, kObject_Type(source), allocator));     
    ...
    kTry
    {
        kTest(xkQueue_Resize(queue, sourceObj->count)); 

        kTest(kCloneItems(obj->itemType, kQueue_DataAt(queue, 0), kQueue_DataAt(source, 0), headCount, kObject_Alloc(queue))); 
        kTest(kCloneItems(obj->itemType, kQueue_DataAt(queue, headCount), kQueue_DataAt(source, headCount), tailCount, kObject_Alloc(queue))); 
    }
    kCatch(&status)
    {
        xkQueue_VDisposeItems(queue); 
        xkQueue_VRelease(queue); 
        kEndCatch(status); 
    }
...
kFx(kBool) xkQueue_VHasForeignData(kQueue queue)
{
    kObj(kQueue, queue); 
    kSize headCount = xkQueue_HeadCount(queue); 
    kSize tailCount = xkQueue_TailCount(queue); 

    return kHasForeignData(obj->itemType, kQueue_DataAt(queue, 0), headCount) || 
           kHasForeignData(obj->itemType, kQueue_DataAt(queue, headCount), tailCount); 
}
...
kFx(kStatus) xkQueue_ReadDat5V1(kQueue queue, kSerializer serializer, kAlloc allocator)
{
    ...
    kCheck(xkQueue_Init(queue, kTypeOf(kQueue), itemType, count, allocator)); 

    kTry
    {
        kTest(xkQueue_Resize(queue, count)); 

        kTest(kSerializer_ReadItems(serializer, itemType, itemVersion, kQueue_DataAt(queue, 0), headCount)); 

        kTest(kSerializer_ReadSize(serializer, &tailCount));
        kTest(kSerializer_ReadType(serializer, &itemType, &itemVersion)); 

        kTest(kSerializer_ReadItems(serializer, itemType, itemVersion, kQueue_DataAt(queue, headCount), tailCount)); 
    }
    kCatch(&status)
    {
        xkQueue_VDisposeItems(queue); 
...
kFx(kStatus) xkQueue_ReadDat6V0(kQueue queue, kSerializer serializer, kAlloc allocator)
{
    kObjR(kQueue, queue); 
    kType itemType = kNULL;            
    kTypeVersion itemVersion; 
