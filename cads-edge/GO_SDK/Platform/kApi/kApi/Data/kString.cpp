...
kBeginClassEx(k, kString) 
    ...
    kAddPrivateVersion(kString, "kdat5", "5.0.0.0", "43-1", WriteDat5V1, ReadDat5V1)
    kAddPrivateVersion(kString, "kdat6", "5.7.1.0", "kString-0", WriteDat6V0, ReadDat6V0)

    //virtual methods
    kAddPrivateVMethod(kString, kObject, VRelease)
    kAddPrivateVMethod(kString, kObject, VInitClone)
...
kFx(kStatus) xkString_VInitClone(kString str, kString source, kAlloc allocator)
{
    kObjR(kString, str); 
    kStatus status = kOK; 

    kCheck(kObject_Init(str, kObject_Type(source), allocator)); 
...
kFx(kStatus) xkString_ReadDat5V1(kString str, kSerializer serializer, kAlloc allocator)
{
    kObjR(kString, str); 
    ...
    kCheck(xkString_Init(str, kTypeOf(kString), kNULL, allocator)); 

    kTry
    {
        kTest(kString_Reserve(str, length)); 

        kTest(kSerializer_ReadItems(serializer, itemType, itemVersion, obj->chars, length)); 
        obj->length = length; 
        obj->chars[length] = 0; 
    }
    kCatch(&status)
...
kFx(kStatus) xkString_ReadDat6V0(kString str, kSerializer serializer, kAlloc allocator)
{
    kObjR(kString, str); 
    kType itemType = kNULL;            
    kTypeVersion itemVersion; 
