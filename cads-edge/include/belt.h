...
namespace cads
{
  ...
  std::function<std::tuple<double,double,double>(PulleyRevolution,double,std::chrono::time_point<std::chrono::system_clock>)> mk_pulley_stats(double init = global_conveyor_parameters.MaxSpeed);
  std::function<int(z_type &,int,int)> mk_profiles_align(int width_n);
  coro<msg,msg> encoder_distance_estimation(Io &next, double stide);
